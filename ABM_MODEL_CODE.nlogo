...
turtles-own
[
  uwierzył?           ;; true, jeśli agent uwierzył plotkę
  ignoruje?          ;; true, jesli agent ignoruje plotkę i jej nie powtarza
  plotka-timer   ;; ilość ticks od kiedy ostatnio agent był sprawdzany na posłuchanie się plotki
  typ                 ;;  "zwykly"  lub  "lider"
  czas-w-plotce     ;; ile ticków agent wierzy w plotkę

 ]

to setup
  clear-all ;; reste pola z poprzednich ustawień
  setup-nodes ;; tworzy agentów
  setup-spatially-clustered-network  ;; tworzenie grupy zarażonych plotką
  ask n-of początkowa-liczba-plotkarzy turtles with [typ = "zwykly"] ;; losowanie grupy zarażonych na początku
    [ become-uwierzył ]
  ask links [ set color white ] ;; kolor linii połączeń
  reset-ticks

end

;; lider tworzy linki do K najbliższych zwykłych agentów
to polacz-lidera [ k ]
  ;; SELF to właśnie utworzony lider
  repeat k [
    let cel min-one-of turtles with [typ = "zwykly" and not link-neighbor? myself] [distance myself]
    if cel != nobody [ create-link-with cel ]
  ]
end
...
to setup-nodes
  ...
  set-default-shape turtles "circle" ;; kształt agentów to kółko
  create-turtles liczba-osób ;; ilość agentów
  ;; --- siewca plotki (jeden) --------------------------------
    ;; --- siewca plotki (jeden) --------------------------------
  ...
  [
    set typ "zwykly"
    setxy (random-xcor * 0.9) (random-ycor * 0.9) ;; odleglosc od lewej/prawej strony i gora/dół
    become-podatny ;; stan podatny na wpływ
    set plotka-timer random plotka-check-frequency
  ]
  create-turtles liczba-liderów [
    set typ "lider"
    set shape "triangle"
    set size 2
    set color cyan
    ...
    set uwierzył? false
    set ignoruje? false
    set plotka-timer 0           ;; nieużywane, ale niech istnieje

  ]
end

to setup-spatially-clustered-network ;; sieć znajomości
  let num-links (średnia-liczba-osób * (liczba-osób + liczba-liderów)) / 2
  ...
  repeat 10
  [
    layout-spring turtles links 0.3 (world-width / (sqrt liczba-osób)) 1
  ]
end

to go
  if all? turtles [not uwierzył?] ;; symualcja konczy się kiedy wszyscy są zdrowi
    [ stop ]
  ask turtles
  [
     set plotka-timer plotka-timer + 1
     if plotka-timer >= plotka-check-frequency
       [ set plotka-timer 0 ]
  ]
  ask turtles with [uwierzył?] [
  set czas-w-plotce czas-w-plotce + 1
]
...
to dodaj-lidera
  create-turtles 1 [
    ...
    polacz-lidera 4                    ;;  ➜ automatyczne linki po kliknięciu
  ]
end



to become-uwierzył  ;; turtle procedure
  set czas-w-plotce 0

  set uwierzył? true
  set ignoruje? false
  if typ = "zwykly" [ set color red ]

end

to become-podatny ;; turtle procedure
  set uwierzył? false
  set ignoruje? false
  if typ = "zwykly" [ set color yellow ]

end

to become-ignoruje  ;; turtle procedure
  set uwierzył? false
  set ignoruje? true
  if typ = "zwykly" [ set color gray ]
  ask my-links [ set color gray - 2 ]
end


to spread-virus
  ask turtles with [uwierzył? and (typ = "zwykly" )]

    [
    ask link-neighbors with [typ = "zwykly" and not ignoruje? and not uwierzył?]
    [
      let liderzy count link-neighbors with [typ = "lider"]    ;; ilu liderów jest sąsiadami agenta

      let modyfikator max list 0 (1 - wplyw-lidera * liderzy)

      let skuteczna prawd-rozpowiedzenia-plotki * modyfikator

      if random-float 100 < skuteczna [
        become-uwierzył
      ]
    ]
  ]
...
to do-virus-checks
  ask turtles with [uwierzył? and plotka-timer = 0]
  [
    if random 100 < prawd-zatrzymania-plotki
    [
      ifelse random 100 < prawd-uodpornienia-na-plotke
        [ become-ignoruje ]
        [ become-podatny ]
    ]
  ]
...
"" ""
PENS
"podatny" 1.0 0 -1184463 true "" "plot (count turtles with [not uwierzył? and not ignoruje?]) / (count turtles) * 100"
"uwierzył" 1.0 0 -2674135 true "" "plot (count turtles with [uwierzył?]) / (count turtles) * 100"
"ignoruje" 1.0 0 -7500403 true "" "plot (count turtles with [ignoruje?]) / (count turtles) * 100"
...
"" ""
PENS
"średni czas" 1.0 0 -4699768 true "" "plot mean [czas-w-plotce] of turtles with [not uwierzył? and czas-w-plotce > 0]"
...
<experiments>
  <experiment name="experiment1" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ignoruje?]</metric>
    <metric>max [czas-w-plotce] of turtles</metric>
    <metric>mean [czas-w-plotce] of turtles</metric>
    <metric>count turtles with [uwierzył?]</metric>
    <metric>ticks</metric>
    <enumeratedValueSet variable="prawd-zatrzymania-plotki">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="liczba-osób">
      <value value="40"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="liczba-liderów">
      <value value="3"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="początkowa-liczba-plotkarzy">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="wplyw-lidera">
      <value value="0"/>
      <value value="0.25"/>
      <value value="0.5"/>
      <value value="0.75"/>
    ...
    <enumeratedValueSet variable="plotka-check-frequency">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="średnia-liczba-osób">
      <value value="6"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="prawd-uodpornienia-na-plotke">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="prawd-rozpowiedzenia-plotki">
      <value value="50"/>
    </enumeratedValueSet>
  </experiment>
